...
def _encrypt_xor(a, b, aes):
    'Returns encrypt(a ^ b).'
    return aes.encrypt(unhexlify('%0.32x' % (int((a), 16) ^ int(hexlify(b), 16))))

def encrypt(privkey,passphrase):
    '''BIP0038 non-ec-multiply encryption. Returns BIP0038 encrypted privkey.'''
    privkeyraw    = bytes(privkey) # bytes
    privkeyhex    = repr(privkey)  # hex
    addr          = format(privkey.uncompressed.address,"BTC")
    salt          = hashlib.sha256(hashlib.sha256(bytes(addr,'ascii')).digest()).digest()[0:4]
    ...
    checksum   = hashlib.sha256(hashlib.sha256(payload).digest()).digest()[:4] # b58check for encrypted privkey
...
def decrypt(encrypted_privkey,passphrase):
    ...
    addr           = format(privkey.uncompressed.address,"BTC")
    saltverify     = hashlib.sha256(hashlib.sha256(bytes(addr,'ascii')).digest()).digest()[0:4]
    if saltverify != salt :
        raise Exception('salt verification failed! Password is likely incorrect.')
    return wif
