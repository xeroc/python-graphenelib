...
class BrainKey(Prefix):
    ...
    def __next__(self):
        """Get the next private key (sequence number increment) for iterators."""
    ...
    def suggest():
        ...
        Randomness is provided by the operating system using ``os.urandom()``.
        """
        word_count = 16
...
class Address(Prefix):
    ...
    def __repr__(self):
        """Gives the hex representation of the ``GrapheneBase58CheckEncoded`` Graphene
        address."""
        return repr(self._address)

    def __str__(self):
        """
        Returns the readable Graphene address.

        This call is equivalent to ``format(Address, "GPH")``
        """
...
class GrapheneAddress(Address):
    """
    Graphene Addresses are different.

    Hence we have a different class
    """
...
class PublicKey(Prefix):
    ...
    def __str__(self):
        """
        Returns the readable Graphene public key.

        This call is equivalent to ``format(PublicKey, "GPH")``
        """
    ...
    def __lt__(self, other):
        """For sorting of public keys (due to graphene), we actually sort according to
        addresses."""
...
class PrivateKey(Prefix):
    ...
    * ``PrivateKey("w-i-f").uncompressed.address``:
    ...
    def derive_private_key(self, sequence):
        """Derive new private key from this private key and an arbitrary sequence
        number."""
    ...
    def derive_from_seed(self, offset):
        ...
        Here, the key itself serves as a `seed`, and `offset` is expected to be a sha256
        digest.
        """
    ...
    def __str__(self):
        """
        Returns the readable (uncompressed wif format) Graphene private key.

        This call is equivalent to ``format(PrivateKey, "WIF")``
        """
