...
class Websocket(Rpc):
    ...
    async def parse_messages(self):
        ...
        Messages which are responses (has id) are stored in dict, while messages which
        are notifies are stored in asyncio queue and are supposed to be processed later
        by whom who sets the subscribtion callback
        """
