...
class MasterPassword(object):
    """
    The keys are encrypted with a Masterpassword that is stored in the
    configurationStore. It has a checksum to verify correctness of the password The
    encrypted private keys in `keys` are encrypted with a random.
    ...
    **masterkey/masterpassword** that is stored in the configuration
    ...
    def locked(self):
        ...
        E.g. Is a valid password known that can be used to decrypt the master key?
        """
    ...
    def unlock(self, password):
        ...
        in the keys database, one must use BIP38, decrypt the masterpassword from the
        configuration store with the user password, and use the decrypted masterpassword
        to decrypt the BIP38 encrypted private keys from the keys storage!
    ...
    def _new_masterpassword(self, password):
        """
        Generate a new random masterkey, encrypt it with the password and store it in
        the store.
    ...
    def _get_encrypted_masterpassword(self):
        ...
        .. note:: The encrypted masterkey is checksummed, so that we can     figure out
        that a provided password is correct or not. The     checksum is only 4 bytes
        long!
        """
