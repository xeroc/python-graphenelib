...
class Account(BlockchainObject, AbstractBlockchainInstanceProvider):
    ...
    def balances(self):
        """
        List balances of an account.

        This call returns instances of
        :class:`amount.Amount`.
        """
    ...
    def balance(self, symbol):
        """
        Obtain the balance of a specific Asset.
    ...
    def history(self, first=0, last=0, limit=-1, only_ops=None, exclude_ops=None):
        """
...
class AccountUpdate(dict, AbstractBlockchainInstanceProvider):
    ...
    through by :class:`notify.Notify`.
