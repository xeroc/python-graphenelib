...
class AbstractGrapheneChain(SyncAbstractGrapheneChain):
    ...
    async def connect(self):
        ...
        Async version does wallet initialization after connect because wallet depends on
        prefix which is available after connection only, and we want to keep __init__()
        synchronous.
        """
    ...
    async def finalizeOp(self, ops, account, permission, **kwargs):
        """
        This method obtains the required private keys if present in the wallet,
        finalizes the transaction, signs it and broadacasts it.
    ...
    async def sign(self, tx=None, wifs=None):
        """
