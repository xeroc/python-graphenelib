...
class Blockchain(SyncBlockchain):
    ...
    async def chainParameters(self):
        """The blockchain parameters, such as fees, and committee-controlled parameters
        are returned here."""
    ...
    async def get_current_block_num(self):
        ...
        .. note:: The block number returned depends on the ``mode`` used           when
        instanciating from this class.
        """
    ...
    async def wait_for_and_get_block(self, block_number, blocks_waiting_for=None):
        """
        Get the desired block from the chain, if the current head block is smaller (for
        both head and irreversible) then we wait, but a.
    ...
    async def awaitTxConfirmation(self, transaction, limit=10):
        """
        Returns the transaction as seen by the blockchain after being included into a
        block.
